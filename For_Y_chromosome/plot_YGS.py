...
for gi in range(0, len(df)):
    # Convert value to numeric float safely, replacing 'NA' or '.' with 0.0
    try:
        df[gi][1] = float(df[gi][1])
    except (ValueError, TypeError):
        df[gi][1] = 0.0

    try:
        df[gi][0] = float(df[gi][0])
    except (ValueError, TypeError):
        df[gi][0] = 0.0
    ...
    elif df[gi][0] >= no_vsck and df[gi][1] < per_vscuk:
        classification.append("Inferred X/A")
    elif df[gi][0] < no_vsck:
        classification.append("Lower than VSCK Threshold")
