...
Usage: bash ipda_target-inference_YGS.sh [-k 15] [-y "het_reads*.fastq.gz"] [-x "hom_reads*.fastq.gz"] [-t target_genome.fasta] [-q 20] [-m INT] [-f INT] [-g INT] [-v 20] [-p 75] [-o path/to/output/]
...
## Path to scripts
YGS=OligoY/scripts/CarvalhoClark2013/YGS.pl
jfBitVector=OligoY/scripts/CarvalhoClark2013/jelly_2_bitvector_mxky_1eg.pl
fastacmd_loose2=OligoY/scripts/CarvalhoClark2013/fastacmd_loose2.awk
plot_YGS=OligoY/scripts/plots/plot_YGS.py
plot_Jelly=OligoY/scripts/plots/plotJellyHisto.py
...
do
    case "${flag}" in
        ...
        ?) echo script usage: bash ipda_target-inference_YGS.sh [-k 15] [-y "het_reads*.fastq.gz"] [-x "hom_reads*.fastq.gz"] [-t target_genome.fasta] [-q 20] [-m INT] [-f INT] [-g INT] [-v 20] [-p 75] [-o path/to/output/] >&2
           exit;;
    esac
done
...
hom_size="300M"
tar_size="300M"
#het_size=$(du -m "${het_fastq}" | cut -f1)"M"
#hom_size=$(du -m "${hom_fastq}" | cut -f1)"M"
#tar_size=$(du -m "${tar_fasta}" | cut -f1)"M"
...
date
echo "## Executing ipda_target-inference_YGS.sh"
...
# --- Homozygote Hash Table ---
if [ ! -f "${out_path}${hom_stem}_q${quality_threshold}_k${k_mer}.jelly" ]; then
    date ## Starting Homozygote Hash Table (k-mers and count) at
    /usr/bin/time -f "# Process TIME: %e || MRES: %M || CPUSK: %S || CPUSU: %U" \
    bash -c "gunzip -c ${hom_fastq} | jellyfish count -t 6 -c 4 -C -L 2 -s ${hom_size} -m ${k_mer} -o ${out_path}${hom_stem}_q${quality_threshold}_k${k_mer}.jelly -Q '${quality}' /dev/stdin"
else
    echo "Skipping Homozygote Jellyfish count: ${out_path}${hom_stem}_q${quality_threshold}_k${k_mer}.jelly already exists."
fi
...
# --- Homozygote Bitvector ---
if [ ! -f "${out_path}${hom_stem}_q${quality_threshold}_L${hom_lower_threshold}_k${k_mer}.bitvector.gz" ]; then
    ...
    echo "Skipping Homozygote Bitvector: ${out_path}${hom_stem}_q${quality_threshold}_L${hom_lower_threshold}_k${k_mer}.bitvector.gz already exists."
fi

# --- Heterozygote Hash Table ---
if [ ! -f "${out_path}${het_stem}_q${quality_threshold}_k${k_mer}.jelly" ]; then
    date ## Starting Heterozygote Hash Table (k-mers and count) at
    /usr/bin/time -f "# Process TIME: %e || MRES: %M || CPUSK: %S || CPUSU: %U" \
    bash -c "gunzip -c ${het_fastq} | jellyfish count -t 6 -c 4 -C -L 2 -s ${het_size} -m ${k_mer} -o ${out_path}${het_stem}_q${quality_threshold}_k${k_mer}.jelly -Q '${quality}' /dev/stdin"
else
    echo "Skipping Heterozygote Jellyfish count: ${out_path}${het_stem}_q${quality_threshold}_k${k_mer}.jelly already exists."
fi
...
# --- Heterozygote Bitvector ---
if [ ! -f "${out_path}${het_stem}_q${quality_threshold}_L${het_lower_threshold}_k${k_mer}.bitvector.gz" ]; then
    ...
    echo "Skipping Heterozygote Bitvector: ${out_path}${het_stem}_q${quality_threshold}_L${het_lower_threshold}_k${k_mer}.bitvector.gz already exists."
fi
...
# Only create symlink for tar_fasta if it comes from another directory
if [ "$(realpath "${tar_fasta}")" != "$(realpath "./$(basename "${tar_fasta}")")" ]; then
    ln -sf ${tar_fasta} .
fi
ln -sf ${out_path}${tar_stem}_L${tar_lower_threshold}_k${k_mer}.bitvector.gz .
ln -sf ${out_path}${hom_stem}_q${quality_threshold}_L${hom_lower_threshold}_k${k_mer}.bitvector.gz .
ln -sf ${out_path}${het_stem}_q${quality_threshold}_L${het_lower_threshold}_k${k_mer}.bitvector.gz .

/usr/bin/time -f "# Process TIME: %e || MRES: %M || CPUSK: %S || CPUSU: %U" \
perl ${YGS} kmer_size=${k_mer} mode=final_run contig=${tar_fasta} \
gen_rep=${tar_stem}_L${tar_lower_threshold}_k${k_mer}.bitvector.gz \
trace=${hom_stem}_q${quality_threshold}_L${hom_lower_threshold}_k${k_mer}.bitvector.gz \
male_trace=${het_stem}_q${quality_threshold}_L${het_lower_threshold}_k${k_mer}.bitvector.gz
mv ${tar_stem}_${hom_stem}_q${quality_threshold}_L${hom_lower_threshold}_k${k_mer}_${het_stem}_q${quality_threshold}_L${het_lower_threshold}_k${k_mer}.final_result ${out_path}${out_stem}_report.ygs
